...
export const handleInstagramCallback = async (req, res) => {
  try {
    ...
    // ==================================================
    // STEP 4: FINAL RESPONSE (🔴 FIX: Token removed here)
    // ==================================================
    const sanitizedAccounts = savedAccounts.map(acc => {
      const accObj = acc.toObject(); // Convert Mongoose document to plain JS object
      delete accObj.access_token;    // Delete the sensitive access token
      return accObj;
    });
    ...
    res.status(200).json({ 
      ...
      accounts: sanitizedAccounts // Send sanitized data to React
    });
...
export const getInstagramStatus = async (req, res) => {
  try {
    // 🔴 FIX: Added .select('-access_token') so DB doesn't fetch the token
    const accounts = await InstagramAccount.find({ userId: req.user._id }).select('-access_token');
