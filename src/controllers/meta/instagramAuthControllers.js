...
export const handleInstagramCallback = async (req, res) => {
  ...
  try {
    ...
    // 🔥 FIX: Reverted to api.instagram.com. This is the ONLY valid endpoint for the initial code exchange.
  ...
  // ==========================================
  try {
    // We keep versioning here because the Graph API requires it to prevent the "method type: get" error
    ...
    // If successful, upgrade our token to the long-lived one
    validAccessToken = longTokenResponse.data.access_token; 
    expiresIn = longTokenResponse.data.expires_in || 5184000; // Usually 60 days
    ...
    // We just keep using the `validAccessToken` from Step A.
  }
...
export const refreshInstagramToken = async (req, res) => {
  ...
  try {
    ...
    const refreshResponse = await axios.get(
      `https://graph.instagram.com/${API_VERSION}/refresh_access_token`, // Added API version
      {
        params: {
...
//       redirect_uri: REDIRECT_URI,
//       response_type: "code",
//       scope: "instagram_business_manage_messages,instagram_business_manage_comments,instagram_business_content_publish",
