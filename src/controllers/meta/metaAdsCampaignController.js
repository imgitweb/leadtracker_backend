...
export const getLinkedPages = async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: "Failed to fetch linked Facebook Pages." });
  }
};
...
export const createFullCampaign = async (req, res) => {
  try {
    ...
    const { 
      ...
      adText, 
      imageUrl,
      targetLocation, // Naya field from frontend
      minAge,         // Naya field from frontend
      maxAge          // Naya field from frontend
    } = req.body;

    // 1. Get Token from DB
    const accountRecord = await MetaAdAccount.findOne({ userId, adAccountId });
    if (!accountRecord || !accountRecord.userAccessToken) {
      return res.status(404).json({ error: "Access Token not found. Please sync your account again." });
    }
    const token = accountRecord.userAccessToken;

    // 2. CREATE CAMPAIGN
    const campaignRes = await axios.post(`${META_API}/${adAccountId}/campaigns`, {
      name: campaignName,
    ...
    const creativeRes = await axios.post(`${META_API}/${adAccountId}/adcreatives`, {
    ...
    const newCampaign = new MetaAdCampaign({
      userId, adAccountId, campaignId, adSetId, creativeId, adId: adRes.data.id,
      campaignName, budget: dailyBudget, websiteUrl
    });
...
export const getPagePosts = async (req, res) => {
  try {
    ...
    const response = await axios.get(`https://graph.facebook.com/v25.0/${pageId}/posts`, {
      params: { 
...
export const deleteCampaign = async (req, res) => {
  try {
    ...
    await MetaAdCampaign.findOneAndDelete({ campaignId: campaignId, userId: userId });
