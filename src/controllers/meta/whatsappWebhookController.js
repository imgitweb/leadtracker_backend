        ...
        if (change.field === "messages") {
          ...
          if (value.statuses && value.statuses.length > 0) {
            for (const statusObj of value.statuses) {
              ...
              // 👇 TERMINAL LOGS FOR EASY DEBUGGING 👇
              console.log("\n==================================================");
              if (status === "failed") {
                  console.log(`🚨🚨 [MESSAGE FAILED] 🚨🚨`);
                  console.log(`📱 Recipient: ${recipientPhone}`);
                  console.log(`❌ Error Message: ${errorMsg}`);
                  console.log(`🔍 Full Error Details:`, JSON.stringify(statusObj.errors, null, 2)); 
              } else if (status === "delivered") {
                  console.log(`✅ [MESSAGE DELIVERED] - To: ${recipientPhone}`);
              } else if (status === "read") {
                  console.log(`👀 [MESSAGE READ] - By: ${recipientPhone}`);
              } else if (status === "sent") {
                  console.log(`➡️ [MESSAGE SENT] - To: ${recipientPhone}`);
              } else {
                  console.log(`ℹ️ [STATUS UPDATE]: ${status.toUpperCase()} - To: ${recipientPhone}`);
              }
              console.log("==================================================\n");
              // 👆 TERMINAL LOGS END 👆
          ...
          // ===================================================================
          // 🔥 INCOMING MESSAGES LOGIC (Jo user text/button bhejta hai)
          // ===================================================================
              ...
              try {
                ...
                if (account && account.ai_enabled && conv.ai_enabled !== false) {
                  ...
                  if (startupContext) {
                    ...
                    try {
                      // Note: Aap pichle controller mein v25.0 use kar rahe the, maine yahan consistency ke liye update nahi kiya hai taaki aapka existing logic na toote. Agar error aaye toh isko v25.0 kar lena.
...
//               // --- 🔥 FIX: Handle Text, Buttons, and Interactive Messages ---
//               let text = "";
              
//               if (msg.type === "text") {
//                 text = msg.text.body;
//               } else if (msg.type === "button") {
//                 // When user clicks a Quick Reply button from a Template
//                 text = msg.button.text;
//               } else if (msg.type === "interactive") {
//                 // When user clicks a List item or Interactive Button
//                 if (msg.interactive.type === "button_reply") {
//                   text = msg.interactive.button_reply.title;
//                 } else if (msg.interactive.type === "list_reply") {
//                   text = msg.interactive.list_reply.title;
//                 }
//               } else {
//                 // Ignore other types like image, audio, video, etc. for now
//                 continue; 
//               }

//               // Agar text empty hai kisi wajah se, toh process mat karo
//               if (!text) continue;
//               // -----------------------------------------------------------
...
//                     // 🔥 Update Conversation & Trigger Socket Alert if a lead was created or re-engaged
//                     if (aiResult.leadAction) {
...
//                       // Save updated conversation with lead info and latest message
//                       conv.last_message = aiReplyText;
//                       conv.last_message_time = new Date();
//                       await conv.save();
